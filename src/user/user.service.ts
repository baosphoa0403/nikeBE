...
export class UserService {
  ...
  async createUser(createUserDto: CreateUserDto): Promise<User> {
    ...
    return await this.userModel.findById(userSave._id,{"password":0}).populate('role');
  }
  ...
  async findOneUser(idUserDto: IdRoleDto): Promise<User> {
    const user = await this.userModel.findById(idUserDto.id,{"password":0}).populate('role');
  ...
  async updateUser(
    ...
    if (!status)
      throw new NotFoundException(`id status: ${updateUserDto.statusId} not found`);
